...
from config import ace2005_config as config
import argparse
import re
...
print(f'category numbers {categories}')
...
def train():
    ...
    for _ in range(config.max_epoches):
        ...
        for train_batch_ind, batch in pbar:
            ...
            avg_f1 = total_f1 / (train_batch_ind + 1)
            ...
            if _ >= config.valid_start_epoch:
                if (train_batch_ind % np.ceil(len(train_loader) * config.valid_portion) == 0 and train_batch_ind != 0) or train_batch_ind == len(train_loader)-1:
                    ...
                    print(
                        f'current test at epoch {_} stage {(train_batch_ind + 1) // (len(train_loader) * config.valid_portion)} p & r & f1 {precision} & {recall} & {avg_f1} {str(best_test)}')
                    model.train()
        if improve_flag:
            no_improvement_epochs = 0
            improve_flag = False
