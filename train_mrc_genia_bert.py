import os
from bert_mrc_pytorch.bert_mrc_global_pointer import BertGlobalPointer, BertGlobalPointerWithLabel, BertEfficientGlobalPointer, BertEfficientGlobalPointerWithLabel
...
tokenizer = BertTokenizerFast.from_pretrained(config.pretrained_model_path, do_lower_case=False)
        ...
        for batch_ind, batch in pbar:
            ...
            if _ >= config.valid_start_epoch:
                if (batch_ind % np.ceil(len(train_loader) * config.valid_portion) == 0 and batch_ind != 0) or batch_ind == len(train_loader)-1:
                    ...
                    print(f'current test at epoch {_} stage {(batch_ind+1) // np.ceil(len(train_loader) * config.valid_portion)} p & r & f1 {precision} & {recall} & {avg_f1} {str(best_test)}')
                    model.train()
