import os
from bert_mrc_pytorch.roberta_mrc_global_pointer import RobertGlobalPointer, RobertGlobalPointerWithLabel, RobertEfficientGlobalPointer, RobertEfficientGlobalPointerWithLabel
...
def load_msra_data(filename, mode='train'):
    ...
    with open(filename, encoding='utf-8') as f:
        l = json.load(f)
    for line in l:
        # line['query'] = query2keyword[line['query']]
...
if config.use_efficient_global_pointer:
    if config.add_label_embedding:
        model = RobertEfficientGlobalPointerWithLabel.from_pretrained(config.pretrained_model_path, head=num_heads,
                                                             head_size=64, label_embedding=label_embedding, dropout=config.dropout_rate)
    else:
        model = RobertEfficientGlobalPointer.from_pretrained(config.pretrained_model_path, head=num_heads,
                                                    head_size=64, dropout=config.dropout_rate)
else:
    if config.add_label_embedding:
        model = RobertGlobalPointerWithLabel.from_pretrained(config.pretrained_model_path, head=num_heads,
                                                    head_size=64, label_embedding=label_embedding, dropout=config.dropout_rate)
    else:
        model = RobertGlobalPointer.from_pretrained(config.pretrained_model_path, head=num_heads,
                                                    head_size=64, dropout=config.dropout_rate)
