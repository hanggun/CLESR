...
from bert_mrc_pytorch.roberta_mrc_global_pointer import RobertGlobalPointer, RobertGlobalPointerWithLabel
...
train_data = load_msra_data(config.train_data_path)
valid_data = load_msra_data(config.dev_data_path)

train_data = train_data + valid_data

test_data = load_msra_data(config.test_data_path)
...
bert_config = RobertaConfig.from_pretrained(config.pretrained_model_path)
model = RobertGlobalPointerWithLabel.from_pretrained(config.pretrained_model_path, head=len(categories),
                                            head_size=64, label_embedding=label_embedding)
model.to(local_rank)
...
def collate_fn(batch):
    ...
    token_ids = torch.LongTensor(sequence_padding(token_ids, length=maxlen))
    segment_ids = torch.LongTensor(sequence_padding(segment_ids, length=maxlen))
    labels = torch.LongTensor(sequence_padding(labels, seq_dims=3))
    mask = torch.LongTensor(sequence_padding(mask, length=maxlen))
...
train_loader = DataLoader(train_dataset, batch_size=config.batch_size, collate_fn=collate_fn, sampler=train_sampler, num_workers=8)
...
def train():
    ...
    for _ in range(config.max_epoches):
        ...
        for batch_ind, batch in pbar:
            texts = batch[0]
            batch = [x.to(local_rank) for x in batch[1:]]
            ...
            pbar.set_postfix(loss=np.mean(avg_loss), f1=np.mean(avg_f1), lr=optimizer.param_groups[0]['lr'])
        ...
        if local_rank == 0:
            ...
            with torch.no_grad():
                for batch_ind, batch in pbar:
                    ...
                    total_f1 += f1.item()
                    avg_f1 = total_f1 / (batch_ind + 1)
                    pbar.set_postfix(f1=np.mean(avg_f1))
...
def evaluate(*args):
    ...
    for batch_ind, batch in pbar:
        ...
        batch = [x.cuda() for x in batch[1:]]
        logits = model(*batch[:3])
        correct, true, pred = global_pointer_f1_score(batch[3], logits)
        total_correct += correct.item()
        total_true += true.item()
        total_pred += pred.item()
        pbar.set_description(f"f1 {2*total_correct/(total_true+total_pred)}")
