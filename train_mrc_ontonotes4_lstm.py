...
from bert_mrc_pytorch.roberta_mrc_global_pointer import LSTMGlobalPointerWithLabel, LSTMGlobalPointer
...
if config.use_efficient_global_pointer:
    ...
    if config.add_label_embedding:
        model = LSTMGlobalPointerWithLabel.from_pretrained(config.pretrained_model_path, head=num_heads,
                                                    head_size=64, label_embedding=label_embedding, dropout=config.dropout_rate)
    else:
        model = LSTMGlobalPointer.from_pretrained(config.pretrained_model_path, head=num_heads,
                                                    head_size=64, dropout=config.dropout_rate)
...
optimizer = torch.optim.Adam(model.parameters(), lr=config.other_lr, eps=config.adam_epsilon)
