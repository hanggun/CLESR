...
from bert_mrc_pytorch.roberta_mrc_global_pointer import RobertGlobalPointer,\
    RobertGlobalPointerWithLabel,\
    RobertEfficientGlobalPointerWithLabel,\
    RobertEfficientGlobalPointer
...
from config import onenote_config as config
import argparse
import re
import random
...
train_data = load_msra_data(config.train_data_path, mode='train')
valid_data = load_msra_data(config.dev_data_path, mode='train')
...
if config.use_efficient_global_pointer:
    if config.add_label_embedding:
        model = RobertEfficientGlobalPointerWithLabel.from_pretrained(config.pretrained_model_path, head=len(categories),
                                                             head_size=64, label_embedding=label_embedding)
    else:
        model = RobertEfficientGlobalPointer.from_pretrained(config.pretrained_model_path, head=len(categories),
                                                    head_size=64)
else:
    if config.add_label_embedding:
        ...
        model = RobertGlobalPointer.from_pretrained(config.pretrained_model_path, head=len(categories),
                                                    head_size=64)
...
def evaluate_v1(*args):
    ...
    with torch.no_grad():
        for batch_ind, batch in pbar:
            ...
            total_true += true.item()
            total_pred += pred.item()
            avg_f1 = 2*total_correct/(total_true+total_pred+1e-10)
            pbar.set_postfix(f1=avg_f1)
